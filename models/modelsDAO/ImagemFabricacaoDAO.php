...
class ImagemFabricacaoDAO
{
   ...
   public function inserir(ImagemFabricacao $imagem)
    ...
    {
        $sql = "SELECT ordem FROM imagem_fabricacao WHERE emprendimento_id = ? ORDER BY ordem ASC";
        $stmt = $this->conn->prepare($sql);
    ...
    {
        $sql = "SELECT * FROM imagem_fabricacao WHERE idImagem = ?";
        $stmt = $this->conn->prepare($sql);
    ...
    {
        $sql = "DELETE FROM imagem_fabricacao WHERE idImagem = ?";
        $stmt = $this->conn->prepare($sql);
